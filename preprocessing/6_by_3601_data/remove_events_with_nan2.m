...
for rat_no=1:length(rats_veh)
    ...
    %% load the data
    cd /Users/pelinozsezer/Documents/Science/Radboud/CBD-Project/binned-data/detections_only_unfiltered
    ...
    % HPCabov
    temp_var = strcat( 'x3=any(isnan(detection_ratID',num2str(rat),'_HPCabov_veh),2);');
    eval(sprintf('%s',temp_var));

    temp_var = strcat( 'detection_ratID',num2str(rat),'_HPCabov_veh(x3,:)=[];');
    eval(sprintf('%s',temp_var));
...
for rat_no=1:length(rats_cbd)
    ...
    % HPCabov
    temp_var = strcat( 'x3=any(isnan(detection_ratID',num2str(rat),'_HPCabov_cbd),2);');
    eval(sprintf('%s',temp_var));

    temp_var = strcat( 'detection_ratID',num2str(rat),'_HPCabov_cbd(x3,:)=[];');
    eval(sprintf('%s',temp_var));
