...
for i=1:length(rats_veh)
    ...
    cd ..
    cd processed_data_unfiltered
    load('processed_HPCabov.mat');
    cd ..
    cd detections_only_unfiltered
    ...
    temp_var = strcat( 'detection_ratID',num2str(rat_no),'_HPCabov_veh=waveforms;');
...
for i=1:length(rats_cbd)
    ...
    temp_var = strcat( 'detection_ratID',num2str(rat_no),'_HPCabov_cbd=waveforms;');
    eval(sprintf('%s',temp_var)); 
